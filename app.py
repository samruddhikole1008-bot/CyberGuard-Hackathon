...
def analyze_url(raw):
    ...
    if host in SHORTENERS:
        score += 25
        reasons.append(
            'A URL-shortening service hides the final destination.'
        )
    ...
    if re.search(r'\d{4,}', host):
        score += 10
        reasons.append(
            'The domain contains a long numeric sequence.'
        )
    ...
    if any(
        ...
        reasons.append(
            ...
            'credential or payment scams.'
        )

    if host in TRUSTED or any(
        host.endswith('.' + d)
        for d in TRUSTED
    ):
        score = max(0, score - 20)
        reasons.append(
            'The domain matches a commonly trusted service.'
        )
...
def analyze_text(text):
    ...
    if re.search(r'\b\d{10}\b', text):
        score += 6
        reasons.append(
            'The message contains a 10-digit number.'
        )

    if 'otp' in text:
        score += 12
        reasons.append(
            'The message asks about an OTP or verification code.'
        )

    if 'click now' in text or 'act now' in text:
        score += 12
        reasons.append(
            'The message uses urgent call-to-action language.'
        )
    ...
    if hits:
        reasons.append(
            'Potentially risky keywords detected: '
            + ', '.join(hits[:8])
            + '.'
        )
